 ...
 * @version $Id: specials.php for group pricing per item 2019-07-21 14:49:16Z webchills $
 */
    ...
    case 'insert':
      if (empty($_POST['products_id'])) {
        ...
        if (substr($specials_price, -1) == '%' || substr($specials_new_products_group_a_price, -1) == '%' || substr($specials_new_products_group_b_price, -1) == '%' || substr($specials_new_products_group_c_price, -1) == '%' || substr($specials_new_products_group_d_price, -1) == '%') {
          $new_special_insert = $db->Execute("select products_id, products_price, products_group_a_price, products_group_b_price, products_group_c_price, products_group_d_price, products_priced_by_attribute
                                              from " . TABLE_PRODUCTS . "
...
// check if priced by attribute
        ...
        $db->Execute("insert into " . TABLE_SPECIALS . "
                     (products_id, specials_new_products_price, specials_new_products_group_a_price, specials_new_products_group_b_price,
                      specials_new_products_group_c_price, specials_new_products_group_d_price, specials_date_added, expires_date, status,
    ...
    case 'update':
      ...
      $specials_price = (!zen_not_null($tmp_value) || $tmp_value == '' || $tmp_value == 0) ? 0 : $tmp_value;
        ...
        if (substr($specials_price, -1) == '%' || substr($specials_new_products_group_a_price, -1) == '%' || substr($specials_new_products_group_b_price, -1) == '%' || substr($specials_new_products_group_c_price, -1) == '%' || substr($specials_new_products_group_d_price, -1) == '%') {
          $new_special_insert = $db->Execute("select products_id, products_price, products_group_a_price, products_group_b_price,
                                              products_group_c_price, products_group_d_price, products_priced_by_attribute
                                              from " . TABLE_PRODUCTS . "
...
<?php
  if ( ($action == 'new') || ($action == 'edit') ) {
    ...
    if ( ($action == 'edit') && isset($_GET['sID']) ) {
      ...
      $product = $db->Execute("select p.products_id, pd.products_name, p.products_price,
                                      p.products_group_a_price, p.products_group_b_price, p.products_group_c_price, p.products_group_d_price,
                                      p.products_priced_by_attribute, s.specials_new_products_price,
                                      s.specials_new_products_group_a_price, s.specials_new_products_group_b_price,
                                      s.specials_new_products_group_c_price, s.specials_new_products_group_d_price,
