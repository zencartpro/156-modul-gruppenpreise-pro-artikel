 ...
 * @version $Id: functions_prices.php for group pricing per item 2019-10-02 15:00:24Z webchills $
 */
...
// bof group pricing per item
    ...
    $product = $db->Execute("select products_price, products_group_a_price, products_group_b_price, products_group_c_price, products_group_d_price, products_model, products_priced_by_attribute from " . TABLE_PRODUCTS . " where products_id = '" . (int)$product_id . "'");


// eof group pricing per item
     ...
     $product_check = $db->Execute("select products_price,  products_group_a_price, products_group_b_price, products_group_c_price, products_group_d_price, products_priced_by_attribute from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
      ...
      if($customers_group) {
        ...
        if( $customers_group == GROUP_PRICE_PER_ITEM1 && $product_check->fields['products_group_a_price'] != 0 or
            ...
            $customers_group == GROUP_PRICE_PER_ITEM4 && $product_check->fields['products_group_d_price'] != 0 ) {
            $products_price = $product_check->fields['products_price'];
